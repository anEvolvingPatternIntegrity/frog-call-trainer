    ...
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-emerald-100 flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md space-y-6">
          <div>
            <h2 className="text-xl font-semibold text-gray-800 mb-1">Start a Quiz</h2>
            <p className="text-sm text-gray-500">Listen to calls and identify the species.</p>
          </div>
          ...
          {selectedRegion && (
            <p className="text-xs text-gray-400">
              You'll be quizzed on all {selectedRegion.species.length} species — all {selectedRegion.species.length} choices shown per question.
            </p>
          )}
          ...
          >
            {mode === 'training' ? 'Start Training' : 'Start Test'}
          </button>
        </div>
      </main>
